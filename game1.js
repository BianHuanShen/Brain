...
const CONFIG = {
    ...
    cellSize: 32,
    baseCoins: 10,
    memorizeTime: 6,       // ⏱️ 6 segundos para memorizar
    countdownTime: 5,      // ⏱️ 5 segundos de cuenta regresiva (5,4,3,2,1)
    maxStars: 3,
    hintCost: 5,
    colors: {
...
function startCountdown() {
    let phase = 'memorize';  // 'memorize' | 'countdown'
    let memorizeTime = CONFIG.memorizeTime;
    let count = CONFIG.countdownTime;
    
    els.countdownOverlay.classList.add('active');
    
    // Mostrar mensaje inicial de memorización
    els.countdownNumber.textContent = 'MEMORIZA';
    els.countdownNumber.style.fontSize = '2rem';
    
    const interval = setInterval(() => {
        if (phase === 'memorize') {
            // Fase de memorización: 6 segundos mostrando la figura
            memorizeTime--;
            
            if (memorizeTime <= 0) {
                // Cambiar a fase de cuenta regresiva
                phase = 'countdown';
                els.countdownNumber.style.fontSize = ''; // Resetear tamaño
                els.countdownNumber.textContent = count;
                animateCountdownNumber();
            } else {
                // Aún memorizando, mostrar tiempo restante opcionalmente
                // o mantener el mensaje de MEMORIZA
                els.countdownNumber.textContent = 'MEMORIZA';
            }
        } else {
            // Fase de cuenta regresiva: 5, 4, 3, 2, 1
            count--;
            ...
            if (count > 0) {
                ...
                // Mostrar "¡YA!" o "GO!"
                els.countdownNumber.textContent = '¡YA!';
                animateCountdownNumber();
            } else {
...
// Animación del número de cuenta regresiva
...
function clamp(val) {
    return Math.max(0, Math.min(CONFIG.size - 1, val));
}
...
function init() {
    ...
    els.btnVerify.addEventListener('click', checkSolution);
